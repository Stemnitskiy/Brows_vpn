# Brows VPN — Индекс документации

> **Обновлено:** 2026-05-22 · Extension **v2.2.1**

---

## Начать здесь

| Документ | Для кого | Описание |
|----------|----------|----------|
| [README.md](./README.md) | Все | Обзор проекта |
| [CURRENT_STATUS.md](./CURRENT_STATUS.md) | Разработчик | **Source of truth** — что работает |
| [SECURITY.md](./SECURITY.md) | Разработчик / ops | P0–P2 hardening |
| [IMPLEMENTATION_ROADMAP.md](./IMPLEMENTATION_ROADMAP.md) | Разработчик | Этапы v1/v2/v3 |
| [QUICK_START.md](./QUICK_START.md) | Разработчик | Сборка и загрузка |
| [RELEASE_CHECKLIST.md](./RELEASE_CHECKLIST.md) | Разработчик | Финальная проверка перед GitHub release |

---

## Планирование

| Документ | Актуальность |
|----------|--------------|
| [CURRENT_STATUS.md](./CURRENT_STATUS.md) | ✅ |
| [IMPLEMENTATION_ROADMAP.md](./IMPLEMENTATION_ROADMAP.md) | ✅ v2.12 security |
| [ROADMAP.md](./ROADMAP.md) | ✅ |
| [GITHUB_RELEASE_HARDENING_ROADMAP.md](./GITHUB_RELEASE_HARDENING_ROADMAP.md) | ✅ подготовка к GitHub |
| [RELEASE_CHECKLIST.md](./RELEASE_CHECKLIST.md) | ✅ release gate |
| [FEATURE_BACKLOG.md](./FEATURE_BACKLOG.md) | ✅ |
| [SECURITY.md](./SECURITY.md) | ✅ |
| [PROJECT_COMPLETE.md](./PROJECT_COMPLETE.md) | 📦 Архив (устаревший заголовок) |
| [DEVELOPMENT_PLAN.md](./DEVELOPMENT_PLAN.md) | 📦 Архив |

---

## Техническая документация

| Документ | Описание |
|----------|----------|
| [ARCHITECTURE.md](./ARCHITECTURE.md) | Архитектура (порт 10808, PAC в extension) |
| [API.md](./API.md) | Native messaging (length-prefixed JSON) |
| [TESTING.md](./TESTING.md) | Тест-план + CI |
| [FINAL_INSTRUCTIONS.md](./FINAL_INSTRUCTIONS.md) | Инструкция пользователя |

---

## Proxy-service

| Документ | Путь |
|----------|------|
| README | [../proxy-service/README.md](../proxy-service/README.md) |
| Native Messaging | [../proxy-service/NATIVE_MESSAGING_SETUP.md](../proxy-service/NATIVE_MESSAGING_SETUP.md) |

---

## Справочные анализы (история)

[CENSORTRACKER_ANALYSIS.md](./CENSORTRACKER_ANALYSIS.md) · [EXTENSION_ANALYSIS.md](./EXTENSION_ANALYSIS.md) · [PROXY_SERVICE_ANALYSIS.md](./PROXY_SERVICE_ANALYSIS.md)

`extension/src/` — reference only, не загружается в Chrome.
