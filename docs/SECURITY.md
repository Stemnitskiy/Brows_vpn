...
- `proxy-service/xray-core/access.log` / `error.log` (legacy/runtime logs, в `.gitignore`)
- `proxy-service/com.browsvpn.host.local.json` (локальный manifest с абсолютным путём)
- `%LOCALAPPDATA%\BrowsVPN\` (legacy, не используется)
- Export JSON с VLESS URL

`access.log` не включается в новых Xray config по умолчанию, потому что содержит посещаемые домены.
...
3. **webRequest** — optional permission только для debug URL tracing; держите debug выключенным.
